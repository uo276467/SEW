<!-- APLICACIÓN DE GESTIÓN DE DATOS 
 El programa crea una base de datos "gestion" con 5 tablas:
 euqipos, pilotos, circuitos, carreras y resultados.
 Importando el archivo gestion.csv de este directorio
 la base de datos se rellena con la información que
 figura en dicho archivo.
 Además de eso la interfaz cuenta con formularios
 para añadir más información a la base de datos. -->
<?php
ob_start();
class DataManager {
    private $server;
    ...
    try {
        ...
        if (!file_exists($filePath)) {
            throw new Exception("El archivo CSV no existe en la ruta especificada: $filePath");
        }
        
        // Intentar abrir el archivo
        $file = fopen($filePath, 'r');
        if (!$file) {
            throw new Exception("No se pudo abrir el archivo CSV. Verifica los permisos del archivo: $filePath");
        }
